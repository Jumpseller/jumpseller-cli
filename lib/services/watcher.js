...
export class ThemeWatcher {
  ...
  isSchemaPath(path) {
    ...
    if (path === "config/theme.json") return true;
    if (path === "config/options.json") return true;
    if (path === "config/settings.json") return true;
    return false;
  }
  ...
  async watch(folder) {
    ...
    this.watcher.on("all", (event, fullpath) => {
      ...
      const action = this.isSchemaPath(relativePath) ? event : "SKIP";
