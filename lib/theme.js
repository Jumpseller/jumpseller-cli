...
const watch = withCurrentStore(async function watch(id, folder, options) {
  if (options.components) options.allow.push("components/*.json");
  if (options.settings) options.allow.push("config/settings.json");
  folder ||= ".";
...
Options --allow, --block, --components and --settings can be used to
control which files should be processed or ignored.
By default writes to components/*.json and config/settings.json files
are blocked as they can be quite destructive.
`.trim();
const WATCH_UNSAFE_FILES = ["components/*.json", "config/settings.json"];
...
program
  ...
  .option("--components", 'Shorthand for --allow "components/*.json"')
  .option("--settings", 'Shorthand for --allow "config/settings.json"')
  .argument("<theme-id>", "Theme id", validateThemeReference)
